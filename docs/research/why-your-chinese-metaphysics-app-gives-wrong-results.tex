% =============================================================================
% Why Your Chinese Metaphysics App Gives Wrong Results
...
% --- Title -------------------------------------------------------------------
\title{%
  \textbf{Why Your Chinese Metaphysics App Gives Wrong Results:}\\[4pt]
  \large A Computational Error Analysis of Solar, Lunar, and Planetary Ephemeris\\
  for \zh{四柱八字}, \zh{奇門遁甲}, \zh{紫微斗數}, \zh{大六壬}, and \zh{七政四餘}%
}
...
\begin{abstract}
\noindent
Five Chinese metaphysical systems---\zh{四柱八字} (Four Pillars of
Destiny), \zh{奇門遁甲} (Strange Gates), \zh{紫微斗數} (Purple Star Astrology),
\zh{大六壬} (Great Six Ren), and \zh{七政四餘} (Seven Luminaries and Four
Remainders)---share a common computational dependency on precise astronomical
ephemeris, yet rely on different celestial mechanics: \zh{八字} and
\zh{奇門遁甲} are fundamentally \emph{solar}, deriving pillar assignments and
formation sequences from the 24 solar terms (\zh{二十四節氣});
\zh{紫微斗數} is fundamentally \emph{lunar}, constructing its fate chart from
the lunar calendar date; \zh{大六壬}'s \zh{十二月將} (twelve Monthly Generals)
bridges both solar and lunar computation; and \zh{七政四餘}, the most astronomically dependent of all,
requires accurate positions of all seven classical celestial bodies (Sun, Moon,
and five planets) in the 28 lunar mansions (\zh{二十八宿}).

This paper identifies and quantifies seven categories of computational error
present in widely-used software for these systems, including the
\zh{子時} day boundary controversy and the competing intercalation
methodologies (\zh{拆補法} vs.\ \zh{置閏法}) in \zh{奇門遁甲}.
We present a three-way statistical validation of the \texttt{stem-branch}
library against \zh{壽星萬年曆} (sxwnl) and JPL~DE441 numerical ephemeris
across 2,284~years (209--2493~CE), demonstrating mean solar term timing
accuracy of $1.05$~seconds ($N = 1{,}008$, max $= 3.05$~s). We further
analyze how the intercalary month (\zh{閏月}) problem---critical for both
\zh{紫微斗數} chart construction and \zh{大六壬} \zh{月將}
determination---depends on the same solar-lunar computational pipeline, and
discuss the irreducible $\Delta T$ uncertainty that limits historical chart
reconstruction. A cross-civilizational analysis of reference meridians---from
the Chinese \zh{天下之中} (Luoyang/Dengfeng) and Indian \textit{deshantara}
correction (Ujjain) to Ptolemy's Fortunate Islands and the 1884 Greenwich
standardization---demonstrates that the reference longitude problem is a
universal feature of coordinate-dependent astronomical traditions, not a
uniquely Chinese difficulty.
\end{abstract}

\medskip
\noindent\textbf{Keywords:} Chinese calendar, BaZi, \zh{奇門遁甲},
\zh{紫微斗數}, \zh{六壬}, \zh{七政四餘}, solar terms, lunar phases,
planetary ephemeris, Swiss Ephemeris, VSOP87, JPL DE441, $\Delta T$,
intercalation, true solar time, prime meridian, reference longitude,
...
Five metaphysical systems depend on this astronomical foundation,
each with distinct computational requirements:

\begin{description}[leftmargin=0pt, labelindent=0pt]
  \item[\zh{四柱八字} (Four Pillars of Destiny)] derives an eight-character
    fate chart from the sexagenary-cycle encoding of birth date and time.
    Year and month pillars are determined by solar term boundaries---the
    year changes at \zh{立春} ($\lambda_\odot = 315\degree$), and months
    at the 12 \zh{節} (sectional) terms. \emph{Primary dependency: solar.}

  \item[\zh{奇門遁甲} (Strange Gates, Hidden Stems)] assigns temporal
    formations (\zh{局}) synchronized to solar term boundaries, with
    competing intercalation methods (\zh{拆補法} vs.\ \zh{置閏法}) that
    impose sub-hour precision requirements.
    \emph{Primary dependency: solar.}

  \item[\zh{紫微斗數} (Purple Star Astrology)] constructs a 12-palace fate
    chart from the \emph{lunar} calendar date, requiring accurate new moon
    computation and intercalary month (\zh{閏月}) determination.
    \emph{Primary dependency: lunar (with solar for \zh{閏月}).}

  \item[\zh{大六壬} (Great Six Ren)] uses \zh{十二月將}
    (twelve Monthly Generals) to map the Sun's zodiacal
    position, while its daily mechanics depend on lunar day counting.
    \emph{Primary dependency: solar and lunar.}

  \item[\zh{七政四餘} (Seven Luminaries and Four Remainders)] charts
    the actual positions of Sun, Moon, and five planets in the 28 lunar
    mansions (\zh{二十八宿}), making it the most astronomically dependent
    of all five systems. \emph{Primary dependency: planetary (all seven
    classical bodies).}
\end{description}

Despite these critical dependencies, the majority of software
applications employ simplified computational methods that introduce errors
ranging from seconds to tens of minutes, silently propagating into incorrect
chart assignments.

This paper makes four contributions:
\begin{enumerate}[label=(\roman*)]
  \item A taxonomy of seven computational error sources
  ...
  \item An analysis of how each metaphysical system depends on the
        solar-lunar-planetary computational pipeline
        (Section~\ref{sec:systems});
  \item An error budget decomposition showing the irreducible $\Delta T$
        uncertainty (Section~\ref{sec:uncertainty}).
\end{enumerate}
...
entire fate chart is constructed from the \emph{lunar} calendar date.
...
% =============================================================================
\section{Astronomical Dependencies of Chinese Metaphysical Systems}
\label{sec:systems}
...
\zh{八字}~\cite{wikipedia_fourpillars} is fundamentally grounded in
\zh{氣} (qi)---the seasonal energy cycle manifested through the
\zh{二十四節氣} and driven by the Sun's annual motion. The correspondence between the five elements (\zh{五行}) and the
seasons---\zh{寒} (cold/winter), \zh{濕} (damp/late summer), \zh{暑}
(heat/summer), \zh{熱} (warmth/early summer)---is an astronomical
phenomenon mapped onto human destiny. The entire BaZi computational pipeline
is therefore \emph{solar}: VSOP87D $\to$ apparent longitude $\to$ solar
...
Calculation'') is a Chinese astrological system attributed to the Daoist
...
The fate chart depends on four lunar-calendar inputs: year, month, day, and
...
software implements this method, but practitioners should be aware that
it differs from both historical alternatives.
...
testify to the depth of Indo-Chinese astronomical exchange. Jeffrey Kotyk's work
...
Islamicate'' \cite{kotyk_2017, kotyk_2018}---a striking parallel with the
...
For the first time in the system's history, \zh{七政四餘} practitioners
receive astronomically correct planetary positions in the 28 mansions.
The astrological framework designed to interpret real celestial geometry
now actually receives real celestial geometry. The parallel with Indian
...
exists. The narrative that accurate positions improve astrological
outcomes is structurally sound---the system was designed for real
positions---but remains formally unconfirmed.
...
Tang Chang'an $\to$ NASA JPL $\to$ modern Chinese astrological software.
...
59.577$~years---tantalizingly close to 60. However, no ancient Chinese
...
boundary cases---precisely where practitioners most need computational
certainty.
...
problem that plagues coordinate-dependent systems like \zh{紫微斗數}.
...
\emph{historically appropriate} ones. A practitioner computing a chart for
the Tang dynasty should arguably use the Dengfeng meridian
...
characteristics for Chinese metaphysical computation:

\begin{enumerate}
  \item \textbf{Local Apparent Solar Time} (pre-17th century):
    Historical practitioners used sundials, so their ``time'' was
...
perhaps the most insidious source of \zh{時辰} misassignment.

East Asian DST history is complex and poorly documented.  Seven regions
observed DST at various times during the 20th century
...
\noindent
The PRC's six years of DST (1986--1991)\footnote{Transition dates and
times are documented in the IANA Time Zone Database (\texttt{tzdata}),
file \texttt{asia}, zone \texttt{Asia/Shanghai}.} alone affect millions
of birth charts.  A birth recorded at 23:30 during DST is actually 22:30 standard
time, changing the \zh{時辰} from \zh{子} (23:00--01:00) to \zh{亥}
(21:00--23:00) and potentially altering the day pillar if the practitioner's
method treats \zh{子時} as crossing the day boundary.  Hong Kong's 33~years
of summer time (HKST $=$ UTC$+$9) mean that \emph{any} \zh{八字}
practitioner computing charts for Hong Kong births between 1946 and 1979
without DST correction has systematically wrong hour pillars for every
summer birth.  South Korea's 12~years include the 1988 Seoul Olympics
period, when many overseas Korean births were recorded.

The practical problem is stark: most Chinese metaphysics practitioners and
apps ignore DST entirely.  Users enter their birth time as displayed on the
...
other time-regime shifts that most practitioners are unaware of:

\textbf{(1)~Japanese occupation timezone shifts.}
During World War~II, Japan imposed JST (UTC$+$9) on all occupied
territories.  Taiwan was shifted from UTC$+$8 to UTC$+$9 on 1~October~1937
and reverted on 21~September~1945.  Hong Kong fell to Japanese time on
25~December~1941 and reverted on 18~November~1945.  Macau was similarly
shifted from 21~December~1941 to 30~September~1945.  Korea, already under
colonial rule, was moved from its native UTC$+$8:30 to JST in 1912
and did not revert until 1945~\cite{china_timezone}.  Any birth chart
computed for these territories during occupation must account for the
forced timezone shift---a correction that most apps silently omit.

\textbf{(2)~Base timezone changes.}
South Korea's base timezone changed \emph{three times} in the 20th century:
...
that confounds naive UTC$+$9 assumptions.  A birth in Seoul on
5~May~1957 at 14:00 wall-clock time (DST active, UTC$+$9:30) is
actually 13:30 standard time (UTC$+$8:30), or equivalently 13:00 in
...
\emph{through the winter} of 1973--1974: DST began 30~December~1973 and
did not end until 20~October~1974---nearly 10 continuous months of
UTC$+$9.  Taiwan reinstated DST in 1974--1975 after a 12-year gap for
the same reason.  These anomalous periods are poorly documented outside
the IANA~\texttt{tzdata} database and are missed by virtually all
Chinese metaphysics software.

\paragraph{Japan's imposed DST (1948--1951).}
Japan's own DST was imposed by US occupation authorities, shifting JST
(UTC$+$9) to JDT (UTC$+$10) during summer months.  It was deeply
...
extensions.  The timezone conversion module%
...
calendars that disagree on specific dates, and practitioners downstream
...
\noindent
\zh{蔡興華}'s revealing public statement---``We have no special techniques
...
modern \zh{通勝} (almanac) traditions---though, ironically, many modern
almanacs continued using precisely the superstitions that the
\zh{協紀辨方書} had painstakingly excised.
...
\subsection{The Practitioner's Error Budget}
...
Practitioners should prioritize: (1)~correct \zh{立春} boundaries,
(2)~true solar time with explicit longitude and Equation of Time corrections,
(3)~DST handling, (4)~explicit day boundary convention,
