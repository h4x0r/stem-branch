...
astronomical fidelity, these systems gained immunity to the table-error
accumulation that nearly destroyed \zh{七政四餘}.

\textbf{The shared chart problem (\zh{共盤問題}).}
\zh{八字}'s algorithmic simplification has a well-known statistical
...
death---diverge dramatically among individuals sharing identical
four-pillar charts.  This is not a subtle philosophical point but a
brute empirical fact verifiable from any birth registry cross-referenced
...
structurally revealing.  Both \zh{七政四餘} (via 7~bodies in 28~mansions)
...
state means it \emph{structurally requires} the practitioner to use the
client's life history, environment, and choices to tune the reading.
...
describe the choice of which plate to apply as \zh{自由心證} (free
conviction---i.e., arbitrary judgment), or in the Cantonese idiom,
``\zh{試到啱為止}'' (keep trying until it fits).  This is a
...
Wang~Tingzhi (\zh{王亭之}), a prominent Hong Kong metaphysics critic,
has documented cases where \zh{鐵板神數} practitioners supplemented
their readings with private investigation of the client's background
rather than genuine divinatory computation.
