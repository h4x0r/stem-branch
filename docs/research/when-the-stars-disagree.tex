...
% Allow up to 3em extra inter-word stretch before declaring overfull \hbox.
% Essential for two-column layout with CJK mixed text and technical terms.
...
to the lunisolar calendar\footnote{The Chinese civil calendar (\zh{農曆}) is \emph{lunisolar}: its months track the Moon (new moon to new moon), while leap months (\zh{閏月}) keyed to the solar terms (\zh{中氣}) keep the year aligned with the tropical cycle. It is widely but imprecisely called the ``lunar calendar'' in English---and \zh{陰曆} (literally ``lunar calendar'') in Chinese---yet a \emph{purely} lunar calendar, such as the Islamic Hijri, applies no solar reconciliation and drifts ${\sim}11$~days per year against the seasons. We use ``lunisolar'' throughout for precision.}'s \zh{閏月} mechanism
(Section~\ref{subsec:error_2033}): both are intercalation strategies that
reconcile incommensurate cycles.

\subsection{\zh{紫微斗數}: The Lunisolar Calendar Dependency}
\label{subsec:ziwei}
...
\begin{enumerate}
  ...
  \item \textbf{Intercalary month determination} (\zh{閏月}): The standard
        ...
        intercalary---linking the lunisolar calendar back to solar term
        computation.
...
lunisolar calendar, effectively converting \zh{紫微斗數} to a solar-based
...
\begin{description}[leftmargin=0pt, labelindent=0pt, nosep]
  \item[\zh{羅睺} (Rahu):] the lunar ascending node, with an 18.6-year
    nodal cycle.
...
reform ``transposed \zh{羅睺} and \zh{計都} and obliterated
...
solar term boundaries). For the lunisolar calendar, it manifests as the rare
...
\emph{lunisolar calendar date}---a discrete integer triple (year, month,
...
the lunisolar date, not by observation~\cite{zwds_quanshu, zwds_quanji}.
...
lunisolar calendar itself requires accurate solar term computation (for
...
\begin{center}
\footnotesize
\begin{tabular}{@{}lp{0.35\columnwidth}l@{}}
\toprule
\textbf{System} & \textbf{Runtime input} & \textbf{Robustness} \\
\midrule
\zh{七政四餘} & 7 planetary positions (sidereal) & Fragile \\
\zh{大六壬} & Solar + lunar conjunction & Moderate \\
\zh{八字} / \zh{奇門遁甲} & Solar terms (tropical) & Robust \\
\zh{紫微斗數} & Lunisolar calendar date (integer) & Maximal \\
\bottomrule
\end{tabular}
\end{center}
\noindent
...
note that the choice of which plate to apply rests on the
practitioner's discretionary judgment rather than on any rule
derivable from the system's axioms.  This constitutes a
degrees-of-freedom problem~\cite{bruun_fengshui}: with three plates and
multiple ring layers, the practitioner commands enough free parameters
to rationalize almost any reading post-hoc.
...
Japan           & 1948--1951                               & 4  \\
S.\ Korea       & 1948--51, 1955--60, 1987--88              & 12 \\
Macau           & 1942--1979 (mirroring HK)                & 34 \\
Mongolia        & 1983--98, 2001--06, 2015--16             & 27 \\
\bottomrule
\end{tabular}
\end{table}
...
% =============================================================================
\section{The Reference Longitude Problem}
\label{sec:meridians}
...
\begin{tabular}{@{}llr@{}}
\toprule
\textbf{Meridian} & \textbf{Used by} & \textbf{$\Delta$ Greenwich} \\
\midrule
...
\begin{itemize}[nosep]
  ...
  \item \textbf{Hybrid}: \zh{紫微斗數} avoids the question entirely
    by using the lunisolar calendar date---a discrete quantity that
...
on Astrology''), Hong Kong: \zh{博益出版集團有限公司} (Bo~Yi
Publishing), 1988.  ISBN 978-962-170-386-6.  Critical essays on
\zh{紫微斗數} and related
divinatory traditions including \zh{鐵板神數}; Wang was the Zhongzhou
School (\zh{中州派}) lineage holder and later Visiting Professor of
Sino-Tibetan Buddhist Studies, Renmin University of China.
